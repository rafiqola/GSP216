...
gcloud beta compute instance-groups managed set-autoscaling "instance-group-1" --zone "us-west1-a" --cool-down-period "45" --max-num-replicas "5" --min-num-replicas "1" --target-cpu-utilization "0.8" --mode "on"


gcloud beta compute instance-templates create instance-template-2 --subnet=projects/$PROJECT_ID/regions/us-west1/subnetworks/subnet-b --metadata=startup-script-url=gs://cloud-training/gcpnet/ilb/startup.sh --region=us-west1 --tags=lb-backend 

gcloud compute instance-groups managed create instance-group-2 --base-instance-name=instance-group-2 --template=instance-template-2 --size=1 --zone=us-west1-b
...
gcloud beta compute health-checks create tcp my-ilb-health-check --region=us-west1 --port=80 --proxy-header=NONE --no-enable-logging --check-interval=5 --timeout=5 --unhealthy-threshold=2 --healthy-threshold=2
...
gcloud compute addresses create my-ilb-ip \
    --region us-west1 --subnet subnet-b --addresses 10.10.30.5
